	...
	const claims = await Claim.find({ userId: req.user.id, ...(status && { status }) })
		.skip((page - 1) * limit)
		.limit(limit);

	const total = await Claim.countDocuments({ userId: req.user.id, ...(status && { status }) });
	...
	interface ClaimsApiResponse extends ApiResponse {
		data: ClaimsData;
	...
	const response: ClaimsApiResponse = {
		...
		data: {
			claims: claims.map((claim: any): ClaimResponse => ({
				...claim.toJSON(),
				formattedAmount: claim.formattedAmount,
				formattedApprovedAmount: claim.formattedApprovedAmount,
				ageInDays: claim.createdAt ? Math.floor((Date.now() - new Date(claim.createdAt).getTime()) / (1000 * 60 * 60 * 24)) : undefined,
			})),
		},
		pagination: {
			page,
	...
	const response: ApiResponse = {
		...
		claim: {
			...claim.toJSON(),
			formattedAmount: formatAmount(claim.amount, claim.currency),
			formattedApprovedAmount: claim.approvedAmount !== undefined ? formatAmount(claim.approvedAmount, claim.currency) : '',
			ageInDays: claim.createdAt ? Math.floor((Date.now() - new Date(claim.createdAt).getTime()) / (1000 * 60 * 60 * 24)) : undefined,
		},
		},
	};
